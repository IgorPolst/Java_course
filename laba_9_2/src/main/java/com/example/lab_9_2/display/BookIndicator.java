package com.example.lab_9_2.display;
...
public class BookIndicator {
    ...
    private final Map<String, Figure> availableBooks = new HashMap<>();
    ...
    public void setup() {
        ...
        availableBooks.putIfAbsent("Main", book);
    }
    ...
    /** Puts (or replaces) a book under the supplied key. */
    public void addBook(String key, Figure newBook) {
        availableBooks.put(key, newBook);
    }

    /** Returns the book stored under {@code key}, or {@code null}. */
    public Figure findByKey(String key) {
        return availableBooks.get(key);
    }

    /* -------------------------------------------------
     *  Convenience console display methods
     * ------------------------------------------------- */
    ...
    public void displayAllBooks() {
        ...
        for (Map.Entry<String, Figure> entry : availableBooks.entrySet()) {
            Figure b = entry.getValue();
            System.out.println(entry.getKey() + ": " + b.getDescription());

            if (dateFormatter != null && b.getPublishDate() != null) {
                System.out.println("  Published: " + dateFormatter.format(b.getPublishDate()));
            }
        }
        System.out.println("==========================");
    }
